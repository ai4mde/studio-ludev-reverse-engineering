...
from scripts.src.test import parse_edge
...
def generate_diagram_json():
                ...
                for field in model._meta.get_fields():
                    if not field.is_relation:
                        ...
                        if is_enum_field(field):
                            ...
                            if field.name not in enum_ptr_map:
                            ...
                            # Set the enum reference for the attribute
                            enum_ref = f'"{enum_ptr_map[field.name]}"'
                        else:
                            attributes.append({
                                "body": None,
                                "enum": enum_ref,
    ...
    #             multiplicity = {
    #                 "source": "1" if field.one_to_many or field.many_to_many else "0..1",
    #                 "target": "*" if field.many_to_many else "1"
    #             }
    #
    #             edge = {
    #                 "id": str(uuid.uuid4()),
    ...
    edges = parse_edge(models_list,model_ptr_map)
